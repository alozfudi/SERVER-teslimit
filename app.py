...
try:
    ...
    import psutil  # Library baru untuk cek RAM
except ImportError:
...
def main():
    ...
    with st.sidebar:
        ...
        try:
            ...
            st.caption(f"RAM Used: {ram.percent}% ({ram.used / (1024**3):.2f} GB / {ram.total / (1024**3):.2f} GB)")
            if ram.percent > 90:
                st.error("⚠️ RAM CRITICAL! Restart App recommended.")
        except Exception:
            st.warning("⚠️ Could not load RAM Monitor")
        # -------------------------
